...
- Casa **sola**: 1 turista cada `spawn_interval` segundos (por defecto **1 seg**).
- Casa **con al menos una casa vecina**: genera al **doble de velocidad** (2 turistas por intervalo).
  - "Vecina" = cualquier casa distinta que toque alguna celda de esta casa en uno de los **4 lados ortogonales** (las diagonales NO cuentan).
  - El bonus no se acumula: 1, 2 o 3 vecinos dan el mismo x2.
- El turista aparece **fuera del terreno**, en un punto al azar de cualquiera de los cuatro bordes (a 1 unidad del límite), y **entra caminando** hacia un punto interior al azar (estado `ENTERING`). Recién al cruzar el borde arranca su ciclo normal de caminar/pausar.
...
1. **Entrando** (`ENTERING`): aparece fuera del borde y camina derecho hacia un punto interior al azar; al entrar al terreno pasa a caminar. Si una casa del borde lo frena, apunta a otro punto interior.
...
- **No se salen del terreno**: al tocar el borde del mapa rebotan hacia adentro.
...
| `scripts/tourist_manager.gd` | Spawner: itera las casas, aplica la regla del vecino, crea turistas |
| `scripts/tourist_menu.gd` | Panel F1 (UI construida por código) |
| `scripts/build_manager.gd` | Casas con `StaticBody3D` (capa 2), señal `buildings_changed`, helpers `get_buildings()` y `house_has_neighbor()` |
